...
public interface ClientDashboardRepository extends AbstractRepository {
	...
	@Query("select stddev(c.budget) from Contract c where c.client.id = :id and c.draftMode = false")
	Double deviationContractsBudget(int id);

	@Query("select min(c.budget) from Contract c where c.client.id = :id and c.draftMode = false")
	Double minimumContractsBudget(int id);

	@Query("select max(c.budget) from Contract c where c.client.id = :id and c.draftMode = false")
